...
struct ClockwiseWebServer
{
  ...
  void getCurrentSettings(WiFiClient client) {
    ...
    char lasts[21];
    tmElements_t tm;
    breakTime(lastNtpUpdateTime(), tm);
    sprintf(lasts, "%02i/%02i/%02i %02i:%02i (UTC)", tm.Day, tm.Month, (1970+tm.Year-2000), tm.Hour, tm.Minute);
    client.printf(HEADER_TEMPLATE_S, "LASTNTP_UPD",lasts);
