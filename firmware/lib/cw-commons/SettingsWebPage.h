...
<body>
  ...
  <div class="w3-bar w3-black w3-medium">
    ...
    <div id="ntp-upd" class="w3-bar-item w3-black w3-hover-green"></div>
    <div id="ssid" class="w3-bar-item w3-button w3-hover-blue w3-right" onclick="resetwifi();"> Reset WiFi</div>
  ...
  <script>
    function createCards(settings) {
      ...
      const cards = [
        ...
        },
        {
          ...
          description: "Name of the description file to be rendered without extension. The local canvas is a serializad json variable on \"localcanvas.h\" file. Available: \"vespa\" and \"snoopy\"",
        ...
        },
        {
          ...
          description: "Server address where the description files are located. Change this to test it locally or set to \"LOCAL\" (all upcase) for a local canvas. (see Description file). default: \"raw.githubusercontent.com\"",
      ...
      document.getElementById("ssid").innerHTML = "<i class='fa fa-wifi'></i> Reset WiFi " + settings.wifissid
      document.getElementById("fw-version").innerHTML = "<i class='fa fa-code-fork'></i> Firmware v" + settings.cw_fw_version
      document.getElementById("ntp-upd").innerHTML = "<i class='fa fa fa-clock-o'></i> Last NTP update " + settings.lastntp_upd
    }
    ...
    function resetwifi() {
      var domanda = confirm("Reset WiFi configuration. Are you sure?");
      if (domanda === true) {
        updatePreference('wifiSsid','none');
        updatePreference('wifiPwd','');
        restartDevice();
      } else {
        alert('Ok, no action');
      }  
